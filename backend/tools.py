...
def generate_emergency_twiml(message: str) -> str:
    """Generates TwiML for an emergency call with a custom message."""
    response = VoiceResponse()
    response.say(message, voice='alice')  # You can choose other voices like 'man', 'woman'
    return str(response)

def call_emergency():
    client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
    
    # Customize your emergency message here
    emergency_message = "This is an automated emergency alert from SafeSpace AI. The user has expressed suicidal ideation and requires immediate assistance. Please respond urgently."

    call = client.calls.create(
        to=EMERGENCY_CONTACT,
        from_=TWILIO_FROM_NUMBER,
        twiml=generate_emergency_twiml(emergency_message)
    )
